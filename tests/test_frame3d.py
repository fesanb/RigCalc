...
class Frame3DTests(unittest.TestCase):
    ...
    def test_axial_bar_matches_closed_form_solution(self):
        ...
        self.assertEqual(diagnostics["method"], "scaled_partial_pivoting")
        self.assertGreater(diagnostics["minimum_scaled_pivot"], 1.0e-12)
        self.assertLess(diagnostics["relative_reduced_residual"], 1.0e-12)

    def test_singular_model_fails_explicitly(self):
        model = FrameModel(
            nodes=[FrameNode("A", 0, 0, 0), FrameNode("B", 1, 0, 0)],
            elements=[FrameElement(
                "E", "A", "B", 100.0e9, 40.0e9, 0.01,
                1.0e-6, 1.0e-6, 1.0e-6)],
        )
        with self.assertRaisesRegex(ValueError, "singular_stiffness_matrix"):
            solve_frame(model)
